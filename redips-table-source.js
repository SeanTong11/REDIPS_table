...
REDIPS.table = (function () {
		...
		for (let t = 0; t < tbl.length; t++) {
			...
			for (let i = 0; i < rc1; i++) {
				...
				for (j = 0; j <= rc2; j++) {
					...
					if (marked === true && mode === 'h') {
						let leftId = (mode === 'v') ? (j + '-' + (i - 1)) : (i + '-' + (j - 1))
						let rightId = (mode === 'v') ? (j + '-' + (i + 1)) : (i + '-' + (j + 1))
						if (cl[leftId] !== undefined && cl[leftId].redips !== undefined) {
							if (cl[leftId].redips.selected === true) {
								c.style["border-left"] = 0
							}
						}
						if (cl[rightId] !== undefined && cl[rightId].redips !== undefined) {
							if (cl[rightId].redips.selected === true) {
								c.style["border-right"] = 0
							}
						}
					}
					if (marked === true && mode === 'v') {
						let topId = (mode === 'v') ? ((j - 1) + '-' + i) : (i + '-' + j)
						let bottomId = (mode === 'v') ? ((j + 1) + '-' + i) : (i + '-' + j)
						if (cl[topId] != undefined && cl[topId] != undefined) {
							if (cl[topId].redips.selected === true) {
								c.style["border-top"] = 0
							}
						}
						if (cl[bottomId] != undefined && cl[bottomId].redips != undefined) {
							if (cl[bottomId].redips.selected === true) {
								c.style["border-bottom"] = 0
							}
						}
					}
	...
	myMerge = function (mode, clear, table) {
		let tbl,		// table array (loaded from tables array or from table input parameter)
			tr,			// row reference in table
			c,			// current cell
			rc1,		// row/column maximum value for first loop
